...
当前系统仅支持管理一个 Minecraft 服务器实例。如果已有实例存在，创建新实例的请求将会失败。

### 解决方案：使用腾讯云子账号

✅ **推荐方案**

创建一个**腾讯云子账号**专门用于 MCSpot 项目，这样可以完美解决上述限制：

- **子账号资源隔离**：子账号只会看到自己有权限的实例，不影响主账号的其他业务
- **精细权限控制**：只授予 CVM 相关的必要权限，提高安全性
- **独立密钥**：使用子账号的 SecretId 和 SecretKey，与主账号隔离
...
**注意**：挂机模式操作需要 `admin` 权限。
开启挂机模式后，状态接口将返回一个虚拟玩家，防止因无人在线导致服务器自动关闭。

#### 设置/关闭挂机模式

设置挂机模式的时长（分钟）。设置为 0 表示关闭挂机模式。
...
| 状态码 | 说明 | 常见原因 |
|--------|------|----------|
| 400 | 请求参数错误 | 缺少必需参数、参数格式错误、已存在实例无法创建等 |
| 401 | 未授权 | 未提供认证信息、用户名或密码错误、认证格式错误 |
| 403 | 权限不足 | 当前用户缺少访问该接口所需的权限 |
| 429 | 请求过于频繁 | 认证失败次数过多，IP已被锁定15分钟 |
| 500 | 服务器内部错误 | 腾讯云API调用失败、Redis连接失败等 |
| 200 | 请求成功 | 请求处理成功，返回相应数据 |

所有错误响应的格式均为：`{"success": false, "message": "错误信息", "error": "详细错误(可选)"}`
